...
To install loops as a Rails plugin you need to do a few steps:
...
Before version 2.0 this code was developed as a Rails plugin only and did not have any 
versions. So we call all those old versions a pre-2.0 releases. If you use one of those (if 
your loops plugin does not have a <tt>VERSION.yml</tt> file in the root), be careful when 
upgrading because there is an incompatible change we have made in 2.0 release: <tt>-h</tt>, 
