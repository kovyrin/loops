...
And then run the +bundle+ command to installe the gem.
...
When you have your loop ready to use, add the following lines to your (maybe empty still)
...
If you want to have more than one copy of your worker running, that is as easy as adding one
...
initialize_loop raises an error, then the loop is not started and the error is logged.
    ...
    class HelloWorldLoop < Loops::Base
      ...
      def self.initialize_loop(config)
        raise "Missing required dependency" unless File.exist?(dependency_file)
      end
