...
module Loops
  class Logger < Delegator
    ...
    def logfile=(logfile)
      ...
      @implementation = LoggerImplementation.new(
        coerced_logfile,
        @number_of_files,
        @max_file_size,
        @write_to_console,
    ...
    class LoggerImplementation < ::Logger
      ...
      def initialize(
        log_device, number_of_files = 10, max_file_size = 10 * 1024 * 1024,
      ...
      def add(severity, message = nil, progname = nil, &)
        ...
        if @write_to_console && (message || progname)
          sev_char = %w[D I W E F A][severity] || 'A'
          puts formatter.call(sev_char, Time.now, progname, message)
        end
