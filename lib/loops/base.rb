...
module Loops
  class Base
    ...
    %i[debug info warn error fatal].each do |meth_name|
    ...
    def with_lock(entity_ids, loop_id, timeout, entity_name = '', &block)
      ...
      entity_ids.each do |entity_id|
        ...
        lock = LoopLock.lock(entity_id:, loop: loop_id.to_s, timeout:)
        unless lock
        ...
        begin
          ...
          LoopLock.unlock(entity_id:, loop: loop_id.to_s)
        end
      end
    end
    ...
    def sleep_with_shutdown_support(seconds)
      ...
      while Time.now.to_f - start_time < seconds
        ...
        begin
          ...
          debug("Sleep terminated with exception: #{e.class}")
          return false
        end
      end
